...
The workflow runs `make check`, verifies the scaffold against the current framework checkout, creates an annotated git tag, creates a GitHub Release without binary or Docker artifacts, and then opens a scaffold update issue for the downstream repository to consume the published tag.
If the tag already exists but the GitHub Release does not, the workflow treats that as a recovery run, verifies the tagged commit, and continues with release publication plus scaffold notification.
...
Configure `SCAFFOLD_REPO_TOKEN` with access to read `zxzharmlesszxz/prometheus-exporter-scaffold` and create issues there before using this automation.
