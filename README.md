...
To publish a module version, update `scaffold/template/go.mod` to the release
version, regenerate `docs/reference.md`, and make the final release-preparation
commit subject exactly `pre-release v0.1.0`. Push CI detects that subject, runs
`make release-preflight VERSION=v0.1.0`, creates an annotated git tag, and
creates a GitHub Release without binary or Docker artifacts. Release notes are
