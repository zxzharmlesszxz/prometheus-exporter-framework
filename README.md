...
Tags are public Go module versions for downstream exporters. Pushes and pull
requests run CI checks, including a scaffold compatibility job that uses the
local scaffold/ template to render a demo exporter against the current framework
checkout.

To publish a module version, run `make release-prepare VERSION=v0.1.0` to update
`scaffold/template/go.mod`, regenerate `docs/reference.md`, and create the final
release-preparation commit with subject exactly `pre-release v0.1.0`. Push CI
detects that subject, runs the normal framework and scaffold checks plus
`make release-preflight VERSION=v0.1.0`, and only then creates an annotated git
tag and GitHub Release without binary or Docker artifacts. Release notes are
generated by GitHub and grouped by `.github/release.yml`.
Use `make push-release VERSION=v0.1.0` to run the local preflight and push the
release-preparation commit to `main`; direct tag pushes should be rejected by
server-side repository rules.

The manual `Release` workflow is kept as a fallback. If a tag exists but the
GitHub Release does not, rerun the manual workflow with the same version to
verify the tagged commit and create the GitHub Release.
