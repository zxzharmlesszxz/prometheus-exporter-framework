...
The workflow runs `make check`, verifies the scaffold against the current framework checkout, creates an annotated git tag, creates a GitHub Release without binary or Docker artifacts, and records release notes generated by GitHub (grouped by `.github/release.yml`).
...
After a release, maintainers should update any downstream scaffold consumers as appropriate.
